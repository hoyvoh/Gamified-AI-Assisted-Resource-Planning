...
The business rules of this application (WFU calculation, COCOMO estimation, genetic optimization, warning logic, developer profiling) are the most valuable and most complex part of the codebase. They must not be buried inside FastAPI route handlers or tangled with SQLAlchemy. Clean Architecture enforces a hard wall between *what the system does* and *how it does it*.
...
│  Orchestration only. Coordinates domain + repos.     │
├─────────────────────────────────────────────────────┤
│  Infrastructure Layer (infrastructure/)              │  ← SQLite, Alembic, LLM clients, HTTP
│  I/O implementations. Concrete adapters.             │
├─────────────────────────────────────────────────────┤
...
**Bounded contexts are fine** — related domain concepts are grouped together under a named context (e.g., `estimation/`, `personnel/`, `scenario/`, `profiling/`). Each context owns its entities, interfaces, and use cases. There is no strict aggregate enforcement — use good judgement to keep related logic together and avoid cross-context coupling.
...
| Scoring engine | `domain/` | `ScoringEngine.score_dimension()` |
| "Assign member to task" use case | `application/` | `AssignmentUseCase.assign()` |
| "Run analysis for member" use case | `application/` | `RunAnalysisUseCase.execute()` |
| Repository interface | `domain/` | `class IScenarioRepository(Protocol)` |
| SQLite query | `infrastructure/` | `SqlScenarioRepository.get_by_id()` |
| LLM prompt execution | `infrastructure/` | `LLMClient.run_prompt()` |
...
### 8. DI wiring (`app/dependencies.py`) — wire the concrete implementations
...
After completing any backend task — before marking it done — verify every applicable point below. If you are working as an agent, either self-verify these or explicitly ask the user to verify if manual/UI checks are needed.

### API endpoints

- [ ] Route returns the correct HTTP status code for success and each error case
- [ ] Pydantic schema validates all required and optional fields correctly
- [ ] Invalid input (missing field, wrong type, out-of-range value) returns 422 with a clear message
- [ ] Business rule violations return the correct error code and message (not 500)
- [ ] Response shape matches the API spec (`backend-spec.md`)

### Business logic

- [ ] Logic lives in `domain/` or `application/` — not inside the router
- [ ] All business rules documented in `business-spec.md` are correctly implemented
- [ ] Edge cases (zero values, null optionals, boundary conditions) are handled
- [ ] No silent swallowing of errors — exceptions are logged or surfaced appropriately

### Database

- [ ] `PRAGMA foreign_keys = ON` is enabled on every connection
- [ ] New columns / tables have a corresponding Alembic migration (`uv run alembic revision --autogenerate`)
- [ ] Migration applies cleanly: `uv run alembic upgrade head`
- [ ] Migration rolls back cleanly: `uv run alembic downgrade -1`
- [ ] All new ORM models are imported in `models/__init__.py`

### Architecture

- [ ] `domain/` imports nothing from `application/`, `infrastructure/`, or `interfaces/`
- [ ] Router contains no `if`/`for` business logic
- [ ] Repository interface is defined in `domain/` and implemented in `infrastructure/`
- [ ] DI is wired in `dependencies.py` — not constructed inline in the router

### Tests

- [ ] Domain logic has a pure unit test (no DB, no HTTP)
- [ ] Use case has an integration test with an in-memory or test-DB repo
- [ ] New API endpoint has at least one happy-path integration test
...
All four checks are **required** before pushing. CI enforces the same gates — a failing check blocks merge.
...
|-------|----------------|
| `ruff format .` | Formatting diffs — auto-fixes in place |
| `ruff check .` | Lint errors (replaces flake8 + isort) |
| `mypy app` | Type errors — strict mode, no `Any` without justification |
| `pytest` | Regressions — domain, application, infrastructure, and interface tests |
